...
public class BookUtil {
	...
	private static boolean checkDate(String publishDate) {
		try {
			...
			Date publishdate = formatter.parse(publishDate);//データ型変換
			String publishstring = formatter.format(publishdate);//ストリング型変換
			
			if (publishDate.equals(publishstring)) {//日付１と日付２が等しいかチェック
				return true;
			} else {
				return false;
			}
